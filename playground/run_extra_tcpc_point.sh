#!/bin/bash
# Slurm wrapper that runs a single TCPC extra-point evaluation.
#
...
#                LOWER_FLARE=... UPPER_FLARE=... sbatch playground/run_extra_tcpc_point.sh
# Usage (local): DIRECTION=... SIGN=... OFFSET=... LOWER_ANGLE=... UPPER_ANGLE=... \
#                LOWER_FLARE=... UPPER_FLARE=... ./playground/run_extra_tcpc_point.sh --local

#SBATCH --job-name=tcpc-extra-point
#SBATCH --time=48:00:00
#SBATCH --cpus-per-task=4
#SBATCH --mem=60G
#SBATCH --output=tcpc-extra-%j.out
...
SCRIPT_PATH="${REPO_ROOT}/playground/evaluate_tcpc_extra_point.py"
...
export TCPC_RUN_TAG="${TCPC_RUN_TAG:-extra_points}"

cd "${REPO_ROOT}"

echo "Running extra-point case ${CASE_LABEL}"
...
python3 -u "${SCRIPT_PATH}" \
    ...
    --job-name "${SLURM_JOB_NAME:-tcpc-extra-point}" \
