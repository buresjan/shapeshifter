#!/bin/bash
# Slurm launcher for the TCPC Nelder–Mead optimisation (TKE objective).
#
# Usage (Slurm):   sbatch playground/optimize_junction_tcpc_tke.sh
# Usage (local):   ./playground/optimize_junction_tcpc_tke.sh --local
#
# The Python script has all parameters embedded; no CLI flags are used.

#SBATCH --job-name=nm-opt-tcpc-tke
#SBATCH --time=180:00:00
#SBATCH --cpus-per-task=8
#SBATCH --gpus=1
#SBATCH --mem=60G
#SBATCH --output=opt-tcpc-tke-nm-%j.out
...
SCRIPT_PATH="${REPO_ROOT}/playground/optimize_junction_tcpc_tke.py"

cd "${REPO_ROOT}"

echo "Launching TCPC Nelder–Mead optimisation (TKE objective)"
