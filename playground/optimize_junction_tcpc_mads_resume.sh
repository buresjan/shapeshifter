#!/bin/bash
# Slurm launcher for the resumed TCPC MADS optimisation.
#
# Usage (Slurm):   sbatch playground/optimize_junction_tcpc_mads_resume.sh
# Usage (local):   ./playground/optimize_junction_tcpc_mads_resume.sh --local
#
# The Python script has all parameters embedded; no CLI flags are used.

#SBATCH --job-name=mads-opt-tcpc-resume
#SBATCH --time=168:00:00
#SBATCH --cpus-per-task=8
#SBATCH --gpus=1
#SBATCH --mem=60G
#SBATCH --output=opt-tcpc-mads-resume-%j.out
...
SCRIPT_PATH="${REPO_ROOT}/playground/optimize_junction_tcpc_mads_resume.py"

cd "${REPO_ROOT}"

echo "Launching resumed TCPC MADS optimisation"
