#!/usr/bin/env python3
"""Evaluate the TCPC objective for a single point and append to a shared CSV.

The script reuses the Nelder–Mead objective pipeline from
``playground/optimize_junction_tcpc.py`` (meshgen geometry → Slurm-submitted
tnl-lbm solver → tcpc_split check). It is meant to run inside a Slurm job,
typically via ``run_extra_tcpc_point.sh``.
"""
...
DEFAULT_CSV = REPO_ROOT / "tmp" / "extra_points" / "extra_points_objectives.csv"
...
def main() -> int:
    ...
    if args.run_tag:
        ...
        os.environ["TCPC_RUN_TAG"] = "extra_points"

    from optimize_junction_tcpc import GEOMETRY_PENALTY, _objective
