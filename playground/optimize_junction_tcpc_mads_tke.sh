#!/bin/bash
# Slurm launcher for the TCPC MADS optimisation (TKE objective).
#
# Usage (Slurm):   sbatch playground/optimize_junction_tcpc_mads_tke.sh
# Usage (local):   ./playground/optimize_junction_tcpc_mads_tke.sh --local
#
...
SCRIPT_PATH="${REPO_ROOT}/playground/optimize_junction_tcpc_mads_tke.py"

cd "${REPO_ROOT}"

echo "Launching TCPC MADS optimisation (TKE objective)"
