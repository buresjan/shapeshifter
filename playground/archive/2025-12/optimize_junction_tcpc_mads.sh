#!/bin/bash
# Slurm launcher for the TCPC MADS optimisation.
#
# Usage (Slurm):   sbatch playground/optimize_junction_tcpc_mads.sh
# Usage (local):   ./playground/optimize_junction_tcpc_mads.sh --local
#
# The Python script has all parameters embedded; no CLI flags are used.

#SBATCH --job-name=mads-opt-tcpc
#SBATCH --time=168:00:00
#SBATCH --cpus-per-task=8
#SBATCH --gpus=1
#SBATCH --mem=32G
#SBATCH --output=opt-tcpc-mads-%j.out
...
SCRIPT_PATH="${REPO_ROOT}/playground/optimize_junction_tcpc_mads.py"

cd "${REPO_ROOT}"

echo "Launching TCPC MADS optimisation"
