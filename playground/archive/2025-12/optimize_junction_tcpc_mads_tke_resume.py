#!/usr/bin/env python3
"""Resume/zoom MADS optimisation of the TCPC junction geometry (TKE objective).

This script is intended as a follow-up to ``optimize_junction_tcpc_mads_tke.py``
when the job is killed by walltime. It:

- Restarts the search from the best point observed so far.
- Tightens the bounds around that point to "zoom in" for local refinement.

The objective pipeline (mesh generation, staging, solver invocation, logging)
is reused from ``optimize_junction_tcpc_tke.py`` so results remain comparable.

Best point from the provided MADS-TKE run:
  eval_id=21, f=221.577526985
  x = [offset=-0.001, lower_angle=-1.4, upper_angle=-5.4, lower_flare=0.00175, upper_flare=0.00095]
"""
...
ALGORITHM_LABEL = "mads_tke_resume"


# ---------------------------------------------------------------------------
# Resume configuration: best point + zoomed bounds
# ---------------------------------------------------------------------------
# Starting point: best parameters seen in the previous run.
X0 = np.array([-1.0e-3, -1.4, -5.4, 1.75e-3, 9.5e-4], dtype=float)

# Zoomed-in bounds around X0 (edit as needed).
# Order matches PARAMETER_NAMES = (offset, lower_angle, upper_angle, lower_flare, upper_flare)
LOWER = np.array(
    [
        -4.0e-3,  # offset
        -3.5,  # lower_angle
...
# Cap evaluations for the refinement run (override with env var if desired).
MAX_EVALS = int(os.environ.get("TCPC_MADS_TKE_RESUME_MAX_EVALS", "70"))
...
def main() -> Tuple[np.ndarray, float]:
    ...
    print("[opt] Starting MADS optimisation (TKE objective, resume/zoom)")
    print(
