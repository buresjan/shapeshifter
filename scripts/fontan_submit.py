#!/usr/bin/env python3
"""Submit a Fontan optimization job via Slurm or run locally."""
...
from fontan_config import PROJECT_ROOT, load_config


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Submit a Fontan optimization job (or run locally).",
...
def main() -> int:
    ...
    job_name = args.job_name or submit_cfg.get("job_name") or "fontan-opt"
    ...
    optimize_cmd = [
        "python3",
        "-u",
        str(PROJECT_ROOT / "scripts" / "fontan_optimize.py"),
        "--config",
